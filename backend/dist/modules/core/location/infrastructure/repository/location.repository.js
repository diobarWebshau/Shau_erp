...
const location_model_1 = require("../orm/location.model");
...
class LocationRepository {
    ...
    findAll = async (query) => {
        ...
        const rows = await location_model_1.LocationModel.findAll({
            where,
            attributes: location_model_1.LocationModel.getAllFields()
        });
        const rowsMap = rows.map((r) => mapModelToDomain(r));
        return rowsMap;
    };
    findById = async (id) => {
        const row = await location_model_1.LocationModel.findByPk(id, {
            attributes: location_model_1.LocationModel.getAllFields()
        });
        return row ? mapModelToDomain(row) : null;
    };
    findByName = async (name) => {
        const row = await location_model_1.LocationModel.findOne({
            where: { name },
            attributes: location_model_1.LocationModel.getAllFields()
        });
        return row ? mapModelToDomain(row) : null;
    ...
    create = async (data) => {
        ...
        try {
            const created = await location_model_1.LocationModel.create(data, { transaction });
            if (!created)
    ...
    update = async (id, data) => {
        ...
        try {
            ...
            const existing = await location_model_1.LocationModel.findByPk(id);
            if (!existing)
                throw new http_error_1.default(404, "La locación que se desea actualizar no fue posible encontrarla.");
            // 2. Aplicar UPDATE
            const [affectedCount] = await location_model_1.LocationModel.update(data, {
                where: { id },
                transaction,
            });
            if (!affectedCount)
            ...
            const updated = await location_model_1.LocationModel.findByPk(id, {
                transaction,
                attributes: location_model_1.LocationModel.getAllFields(),
            });
            await transaction.commit();
    ...
    delete = async (id) => {
        ...
        try {
            ...
            const deleted = await location_model_1.LocationModel.destroy({
                where: { id },
                transaction,
            });
            if (!deleted)
