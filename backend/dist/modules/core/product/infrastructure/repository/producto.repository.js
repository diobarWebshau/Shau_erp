...
const product_model_1 = require("../orm/product.model");
...
class ProductRepository {
    ...
    findAll = async (query) => {
        ...
        const rows = await product_model_1.ProductModel.findAll({
            where,
            attributes: product_model_1.ProductModel.getAllFields(),
        });
        return rows.map(pl => mapModelToDomain(pl));
    };
    findById = async (id) => {
        const row = await product_model_1.ProductModel.findByPk(id, {
            attributes: product_model_1.ProductModel.getAllFields()
        });
        return row ? mapModelToDomain(row) : null;
    };
    findByName = async (name) => {
        const row = await product_model_1.ProductModel.findOne({
            where: { name },
            attributes: product_model_1.ProductModel.getAllFields()
        });
        return row ? mapModelToDomain(row) : null;
    ...
    create = async (data) => {
        ...
        try {
            const created = await product_model_1.ProductModel.create(data, { transaction });
            if (!created)
    ...
    update = async (id, data) => {
        ...
        try {
            ...
            const existing = await product_model_1.ProductModel.findByPk(id);
            if (!existing)
                throw new http_error_1.default(404, "El producto que se desea actualizar no fue posible encontrarlo.");
            // 2. Aplicar UPDATE
            const [affectedCount] = await product_model_1.ProductModel.update(data, {
                where: { id },
                transaction,
            });
            if (!affectedCount)
            ...
            const updated = await product_model_1.ProductModel.findByPk(id, {
                transaction,
                attributes: product_model_1.ProductModel.getAllFields(),
            });
            await transaction.commit();
    ...
    delete = async (id) => {
        ...
        try {
            ...
            const deleted = await product_model_1.ProductModel.destroy({
                where: { id },
                transaction,
            });
            if (!deleted)
