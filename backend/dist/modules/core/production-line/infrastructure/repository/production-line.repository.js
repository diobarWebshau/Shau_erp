...
const production_lines_model_1 = require("../orm/production-lines.model");
...
class ProductionLineRepository {
    ...
    // ================================================================
    findAll = async () => {
        const rows = await production_lines_model_1.ProductionLineModel.findAll({
            attributes: production_lines_model_1.ProductionLineModel.getAllFields()
        });
        const rowsMap = rows.map((pl) => mapModelToDomain(pl));
        return rowsMap;
    };
    findById = async (id) => {
        const row = await production_lines_model_1.ProductionLineModel.findByPk(id, {
            attributes: production_lines_model_1.ProductionLineModel.getAllFields()
        });
        return row ? mapModelToDomain(row) : null;
    };
    findByName = async (name) => {
        const row = await production_lines_model_1.ProductionLineModel.findOne({
            where: { name },
            attributes: production_lines_model_1.ProductionLineModel.getAllFields()
        });
        return row ? mapModelToDomain(row) : null;
    ...
    create = async (data) => {
        ...
        try {
            const created = await production_lines_model_1.ProductionLineModel.create(data, { transaction });
            if (!created)
    ...
    update = async (id, data) => {
        ...
        try {
            ...
            const existing = await production_lines_model_1.ProductionLineModel.findByPk(id);
            if (!existing)
                throw new http_error_1.default(404, "La línea de producción que se desea actualizar no fue posible encontrarla.");
            // 2. Aplicar UPDATE
            const [affectedCount] = await production_lines_model_1.ProductionLineModel.update(data, {
                where: { id },
                transaction,
            });
            if (!affectedCount)
            ...
            const updated = await production_lines_model_1.ProductionLineModel.findByPk(id, {
                transaction,
                attributes: production_lines_model_1.ProductionLineModel.getAllFields(),
            });
            await transaction.commit();
    ...
    delete = async (id) => {
        ...
        try {
            ...
            const deleted = await production_lines_model_1.ProductionLineModel.destroy({
                where: { id },
                transaction,
            });
            if (!deleted)
