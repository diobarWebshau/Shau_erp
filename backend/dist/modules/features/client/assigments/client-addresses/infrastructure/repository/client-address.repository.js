...
const client_address_model_1 = require("../orm/client-address.model");
...
class ClientAddressRepository {
    ...
    // ================================================================
    findAll = async () => {
        const rows = await client_address_model_1.ClientAddressModel.findAll({
            attributes: client_address_model_1.ClientAddressModel.getAllFields()
        });
        const rowsMap = rows.map((pl) => mapModelToDomain(pl));
        return rowsMap;
    };
    findById = async (id) => {
        const row = await client_address_model_1.ClientAddressModel.findByPk(id, {
            attributes: client_address_model_1.ClientAddressModel.getAllFields()
        });
        return row ? mapModelToDomain(row) : null;
    };
    findByClientId = async (client_id) => {
        const row = await client_address_model_1.ClientAddressModel.findOne({
            where: { client_id },
            attributes: client_address_model_1.ClientAddressModel.getAllFields()
        });
        return row ? mapModelToDomain(row) : null;
    ...
    create = async (data) => {
        ...
        try {
            const created = await client_address_model_1.ClientAddressModel.create(data, { transaction });
            if (!created)
    ...
    update = async (id, data) => {
        ...
        try {
            ...
            const existing = await client_address_model_1.ClientAddressModel.findByPk(id);
            if (!existing)
                throw new http_error_1.default(404, "La dirección del cliente que se desea actualizar no fue posible encontrarlo.");
            // 2. Aplicar UPDATE
            const [affectedCount] = await client_address_model_1.ClientAddressModel.update(data, {
                where: { id },
                transaction,
            });
            if (!affectedCount)
            ...
            const updated = await client_address_model_1.ClientAddressModel.findByPk(id, {
                transaction,
                attributes: client_address_model_1.ClientAddressModel.getAllFields(),
            });
            await transaction.commit();
    ...
    delete = async (id) => {
        ...
        try {
            ...
            const deleted = await client_address_model_1.ClientAddressModel.destroy({
                where: { id },
                transaction,
            });
            if (!deleted)
