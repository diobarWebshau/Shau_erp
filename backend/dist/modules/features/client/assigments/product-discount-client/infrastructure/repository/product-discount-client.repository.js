...
const product_discount_client_model_1 = require("../orm/product-discount-client.model");
...
class ProductDiscountClientRepository {
    ...
    // ================================================================
    findAll = async () => {
        const rows = await product_discount_client_model_1.ProductDiscountClientModel.findAll({
            attributes: product_discount_client_model_1.ProductDiscountClientModel.getAllFields()
        });
        const rowsMap = rows.map((r) => mapModelToDomain(r));
        return rowsMap;
    };
    findById = async (id) => {
        const row = await product_discount_client_model_1.ProductDiscountClientModel.findByPk(id, {
            attributes: product_discount_client_model_1.ProductDiscountClientModel.getAllFields()
        });
        console.log("diobar");
    ...
    findByProductClientId = async (product_id, client_id) => {
        const row = await product_discount_client_model_1.ProductDiscountClientModel.findOne({
    ...
    create = async (data) => {
        ...
        try {
            const created = await product_discount_client_model_1.ProductDiscountClientModel.create(data, { transaction });
            if (!created)
    ...
    update = async (id, data) => {
        ...
        try {
            ...
            const existing = await product_discount_client_model_1.ProductDiscountClientModel.findByPk(id);
            if (!existing)
            ...
            // 2. Aplicar UPDATE
            const [affectedCount] = await product_discount_client_model_1.ProductDiscountClientModel.update(data, {
                where: { id },
                transaction,
            });
            if (!affectedCount)
            ...
            const updated = await product_discount_client_model_1.ProductDiscountClientModel.findByPk(id, {
                transaction,
                attributes: product_discount_client_model_1.ProductDiscountClientModel.getAllFields(),
            });
            await transaction.commit();
    ...
    delete = async (id) => {
        ...
        try {
            ...
            const deleted = await product_discount_client_model_1.ProductDiscountClientModel.destroy({
                where: { id },
                transaction,
            });
            if (!deleted)
