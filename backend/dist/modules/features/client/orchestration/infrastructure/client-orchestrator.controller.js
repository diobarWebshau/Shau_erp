...
class ClientOrchestratorController {
    ...
    create = async (req, res) => {
        ...
        const result = await this.createClientOrchestratorUseCase.execute(payload);
        return res.status(201).json(result);
    };
    update = async (req, res) => {
        const { id } = req.params;
        const { payload } = req.body;
        const result = await this.updateClientOrchestratorUseCase.execute(Number(id), payload);
