...
const product_process_model_1 = require("../orm/product-process.model");
...
class ProductProcessRepository {
    ...
    // ================================================================
    findAll = async () => {
        const rows = await product_process_model_1.ProductProcessModel.findAll({
            attributes: product_process_model_1.ProductProcessModel.getAllFields()
        });
        const rowsMap = rows.map((r) => mapModelToDomain(r));
        return rowsMap;
    };
    findById = async (id) => {
        const row = await product_process_model_1.ProductProcessModel.findByPk(id, {
            attributes: product_process_model_1.ProductProcessModel.getAllFields()
        });
    ...
    create = async (data) => {
        ...
        try {
            const created = await product_process_model_1.ProductProcessModel.create(data, { transaction });
            if (!created)
    ...
    update = async (id, data) => {
        ...
        try {
            ...
            const existing = await product_process_model_1.ProductProcessModel.findByPk(id);
            if (!existing)
            ...
            // 2. Aplicar UPDATE
            const [affectedCount] = await product_process_model_1.ProductProcessModel.update(data, {
                where: { id },
                transaction,
            });
            if (!affectedCount)
            ...
            const updated = await product_process_model_1.ProductProcessModel.findByPk(id, {
                transaction,
                attributes: product_process_model_1.ProductProcessModel.getAllFields(),
            });
            await transaction.commit();
    ...
    delete = async (id) => {
        ...
        try {
            ...
            const deleted = await product_process_model_1.ProductProcessModel.destroy({
                where: { id },
                transaction,
            });
            if (!deleted)
