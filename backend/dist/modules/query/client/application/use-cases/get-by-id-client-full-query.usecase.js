 ...
 */
class GetByIdClientsFullQueryUseCase {
    repo;
    constructor(repo) {
        this.repo = repo;
    }
    async execute(id, tx) {
        const clientRecord = await this.repo.getByIdClientFullQuery(id, tx);
        if (!clientRecord)
            return null;
...
exports.GetByIdClientsFullQueryUseCase = GetByIdClientsFullQueryUseCase;
;
