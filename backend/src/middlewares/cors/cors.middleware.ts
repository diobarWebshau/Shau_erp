import type { CorsRequest, CorsOptionsDelegate } from "cors";
 ...
 */
const ACCEPTED_ORIGINS: Set<string> = new Set<string>([
    "http://localhost:3003",
    "http://localhost:5173",
    "http://localhost:5174",
]);
 ...
 */
const corsDelegate: CorsOptionsDelegate<CorsRequest> = (req, callback): void => {
     ...
     */
    if (origin === undefined) return callback(null, { origin: true });
    /* -------------------------------------------------------------------------
     * CASO 2 — Origen permitido explícitamente
     * -------------------------------------------------------------------------
     * Aquí garantizamos que el origen viene en nuestra lista blanca.
     */
    if (ACCEPTED_ORIGINS.has(origin)) return callback(null, { origin: true });
    /* -------------------------------------------------------------------------
