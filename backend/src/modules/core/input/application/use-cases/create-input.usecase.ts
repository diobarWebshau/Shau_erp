import type { InputCreateProps, InputProps } from "../../domain/input.types";
    ...
    async execute(data: InputCreateProps): Promise<InputProps> {
        ...
        if (data?.name) {
            ...
            if (existsByName) {
                throw new HttpError(
                    409,
                    "El nombre ingresado para el nuevo insumo, ya esta utilizado por otro insumo."
                );
            }
        }
        ...
        if (data?.sku) {
            ...
            if (existsBySku) {
                throw new HttpError(
                    409,
                    "El sku ingresado para el nuevo insumo, ya esta utilizado por otro insumo."
                );
            }
        }
        ...
        if (data?.custom_id) {
            ...
            if (existsByCustomId) {
                throw new HttpError(
                    409,
                    "El id único ingresado para el nuevo insumo, ya esta utilizado por otro insumo."
                );
            }
        }
        ...
        if (data?.barcode) {
            ...
            if (existsByBarcode) {
                throw new HttpError(
                    409,
                    "El codigo de barras ingresado para el nuevo insumo, ya esta utilizado por otro insumo."
                );
            }
        }
        ...
        // ------------------------------------------------------------------
        const created: InputProps = await this.repo.create(data);
