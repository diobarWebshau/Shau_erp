import {
    getAllProductionLinesSchema, getByIdProductionLineSchema, getByNameProducionLineSchema,
    getByCustomIdProducionLineSchema, createProducionLineSchema, updateProducionLineSchema,
...
export const productionLineRouter = (): Router => {
    ...
    const controller: ProductionLineController = new ProductionLineController();
    router.get("/", validateRequest(getAllProductionLinesSchema), controller.getAll);
    router.get("/id/:id", validateRequest(getByIdProductionLineSchema), controller.getById);
    router.get("/name/:name", validateRequest(getByNameProducionLineSchema), controller.getByName);
    router.get("/custom_id/:custom_id", validateRequest(getByCustomIdProducionLineSchema), controller.getByCustomId);
    router.post("/", validateRequest(createProducionLineSchema), controller.create);
    router.patch("/:id", validateRequest(updateProducionLineSchema), controller.update);
    router.delete("/:id", validateRequest(deleteProducionLineSchema), controller.delete);
    return router;
}
