...
export class ItemController {
    ...
    constructor() { this.repo = new ItemRepository(); }
    getAll = async (_req: ApiRequest<GetAllItemSchema>, res: ApiResponse<GetAllItemSchema>) => {
        const items: ItemProps[] = await this.repo.findAll();
        return res.status(200).send(items);
    };
    getById = async (req: ApiRequest<GetByIdItemSchema>, res: ApiResponse<GetByIdItemSchema>) => {
        const { id }: GetByIdItemSchema["params"] = req.params;
        const items: ItemProps | null = await this.repo.findById(Number(id));
        return res.status(200).send(items);
    };
};