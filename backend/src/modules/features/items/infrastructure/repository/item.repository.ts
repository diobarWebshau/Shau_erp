...
import { ItemModel } from "../orm/item.orm";
...
const mapModelToDomain = (model: ItemModel): ItemProps => {
    const json: ItemProps = model.toJSON();
    return {
        id: json.id,
...
export class ItemRepository implements IItemRepository {
    findAll = async (): Promise<ItemProps[]> => {
        const itemResponses: ItemModel[] = await ItemModel.findAll();
        return itemResponses.map((i) => mapModelToDomain(i));
    };
    findById = async (id: number): Promise<ItemProps | null> => {
        const itemResponse: ItemModel | null = await ItemModel.findByPk(id);
        return itemResponse ? mapModelToDomain(itemResponse) : null;
    }
};