...
export class GetByProductionLineProductUseCase {
    private readonly repo: IProductionLineProductRepository;
    constructor(productionLineRepo: IProductionLineProductRepository) {
        this.repo = productionLineRepo;
    };
    async execute(production_line_id: number, product_id: number, tx?: Transaction) {
        const plp: ProductionLineProductProps | null = await this.repo.findByProductionLineProduct(production_line_id, product_id, tx);
        return plp;
    };
}