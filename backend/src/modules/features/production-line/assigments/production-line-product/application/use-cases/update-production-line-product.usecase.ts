...
export class UpdateProductionLineProductUseCase {
    constructor(private readonly repo: IProductionLineProductRepository) { }
    async execute(id: number, data: ProductionLineProductUpdateProps, tx?: Transaction): Promise<ProductionLineProductProps> {
        const existing: ProductionLineProductProps | null = await this.repo.findById(id, tx);
