...
export class GetProductProcessByIdProductProcessUseCase {
    constructor(private readonly repo: IProductProcessRepository) { }
    execute = async (product_id: number, process_id: number, tx?: Transaction): Promise<ProductProcessProps | null> => this.repo.findByIdProductInput(product_id, process_id, tx);
};
