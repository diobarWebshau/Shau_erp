import { IInventoryQueryRepository } from "../../domain/inventory-query.repository.interface";
...
export class GetByIdInventoryQueryUseCase {

    private readonly repo: IInventoryQueryRepository;

    constructor(repo: IInventoryQueryRepository) {
        this.repo = repo;
    };

    execute = async (inventory_id: number, tx?: Transaction): Promise<InventoryQueryProps | null> => {
        const responseInventory: InventoryQueryProps | null = await this.repo.findByInventoryId(inventory_id, tx);
