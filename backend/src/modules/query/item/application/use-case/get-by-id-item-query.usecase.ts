...
import { ItemRepository } from "@src/modules/features/items/infrastructure/repository/item.repository";
...
import { ItemQueryProps } from "../../domain/item-query.types";
...
interface IGetByIdItemQueryUseCase {
    productRepo: IProductRepository,
    inputRepo: IInputRepository,
    itemRepo: IItemRepository
}

export class GetByIdItemQueryUseCase {

    private readonly productRepo: IProductRepository;
    private readonly itemRepo: ItemRepository;
    private readonly inputRepo: IInputRepository;

    constructor({ inputRepo, productRepo, itemRepo }: IGetByIdItemQueryUseCase) {
        this.inputRepo = inputRepo;
        this.itemRepo = itemRepo;
        this.productRepo = productRepo;
    };

    execute = async (id: number): Promise<ItemQueryProps | null> => {
        const item: ItemProps | null = await this.itemRepo.findById(id);
        if (item) {
            let itemWithRelation: ItemQueryProps;
            const { item_id, item_type } = item;
            if (item_type === "product") {
                const product: ProductProps | null = await this.productRepo.findById(item_id);
                itemWithRelation = {
                    ...item,
                    item: product
                };
            } else {
                const input: InputProps | null = await this.inputRepo.findById(item_id);
                itemWithRelation = {
                    ...item,
                    item: input
                };
